...
public class PestanaMainFav extends Activity {
	...
	private class ListenerRun implements OnCheckedChangeListener {
		...
		public void onCheckedChanged(CompoundButton boton, boolean estado) {
			if (estado && !isMyServiceRunning()) {// encendemos
				...
				Toast.makeText(PestanaMainFav.this, "Alarma iniciada",
				...
				Toast.makeText(PestanaMainFav.this, "Alarma detenida",
						Toast.LENGTH_SHORT).show();
			}
	...
	private void iniciaServicio(Servidor s) {
		...
		if (!SingletonServicios.getConexion().getHosts().contains(s)) {

			SingletonServicios.getConexion().getHosts().add(s);
			cantidad.setText("servicios :"
					+ SingletonServicios.getConexion().getHosts().size());
		}
	...
	private void paraServicio(Servidor s) {
		...
		SingletonServicios.getConexion().getHosts().remove(s);
		cantidad.setText("servicios :"
				+ SingletonServicios.getConexion().getHosts().size());
		...
		if (requestCode == REQUEST_FORMULARIO) {
			...
			if (requestCode == REQUEST_CONTEXT) {
				if (resultCode == Activity.RESULT_OK) {
					...
					if (estabaactivo) {
						SingletonServicios.getConexion().getHosts().add(serv);
						estabaactivo = false;
					}
					adapter.notifyDataSetChanged();
					cantidad.setText("servicios :"
	...
	private class ArrayAdapterServidor extends ArrayAdapter<Servidor> {
		...
		public View getView(final int position, View convertView,
			...
			if (SingletonServicios.getConexion().getHosts()
